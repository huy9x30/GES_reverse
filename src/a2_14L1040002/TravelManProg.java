...
public class TravelManProg {
    ...
    private void inputNoStar(Hotel h) {
        System.out.print("Enter hotel's number of stars: ");
    ...
    public void showMenu() {
        ...
        String option = TextIO.getln();
            switch(option) {
                case "1":             
                    System.out.println("= Show all hostels =");
                    this.displayReport(hostels); 
                    break;
                case "2": 
                    this.displayReport(this.searchHostels()); 
                    break;
    ...
    public SortedSet searchHostels() throws NotPossibleException {
        // TODO:
        // stub
        System.out.println("= Search for hostels by name or location");
        String keywords = TextIO.getln();
        if (keywords.equals(null) || keywords.equals("")) {
            throw new NotPossibleException("Keyword is not allowed to be null or empty.");
        }
        System.out.println("> Searching for hostels with ‘" + keywords + "...");
        SortedSet matchElements = new SortedSet();
        for (Hostel hotel : hostels) {
            if (hotel.getName().contains(keywords) || hotel.getAddress().contains(keywords)) {
                matchElements.add(hotel);
            }
        }
